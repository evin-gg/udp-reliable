...
def update_data():
    ...
    with open("./logs/proxy.log", "r") as file:
        for line in file:
            line = line.strip()
            if line == "[ACK]":
                acks += 1
        ...
        os.system('clear')
        print("[PROXY LOGGER] CURRENT STATISTICS")
        print("\nSENT: ", sent)
        print("ACKS: ", acks)
        file.close()
...
if __name__ == "__main__":
    ...
    filechanges = threading.Thread(target=detect_file_changes, args=("./logs/proxy.log",))
    filechanges.start()
    ...
    plt.title("Proxy Packet Statistics")
